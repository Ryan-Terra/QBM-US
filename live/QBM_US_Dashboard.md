...
**Full QBM run on the 15 Tech Services names (20 Aug 2026) — final result: 8 of 15 Add.** [Full record](../screens/QBM_US_FullQBM_2026-08-20_TechServices.md). Insider Ownership & Net Buying Screen went through three iterations: net buying required (v1.13, 0/15) → "no material net selling" as a hard gate (v1.14, 2/15) → **net selling demoted to a disclosed factor moving Decision Confidence/Priority Tier, ownership kept as the only hard gate (v1.15)**. The six-name ownership data gap (KN, DLB, CNXN, DGII, PI, ARLO) was then closed: **DLB (~35% via the Dolby family's Class B stake), CNXN (Gallup alone owns 10%+), DGII (2.6-5.78%) and PI (2.5%) all clear the ownership gate and now Advance; KN (1.7%) and ARLO (4.03%) confirmed failing it.** Final 8 Adds: **BDC (Ready, no discount — the cleanest of the eight), CNXN/DLB/AGYS (Ready with Considerations), NTCT/DGII (Await Better Entry — both fail the ATH rule), CTS (Await Better Entry, Medium-Low confidence), PI (Ready with Considerations, Low confidence — the most severe insider selling found in the whole batch).** None has a target-portfolio slot — all 8 join the bench (12 total, with NEO, HRMY, KRYS, HQY). 7 remain Not Advanced: 5 confirmed-failing ownership (ACIW, PLXS, HLIT, KN, ARLO), 2 blocked by independent securities-litigation findings (RAL, CVLT).
...
| QBM-US Methodology Version | 1.15 |
| Investment Charter Status | **Permanent and controlling** — Version 1.3 (amended 19 Aug 2026: 6-18 month Target Holding Period; Entry-Point Discipline; Portfolio Concentration — 5-8 holdings, 10-20% each) |
| Operating-System Release | **QBM-US Version 1.15** |
...
| Item | Current status |
|---|---|
| Current Committee Position | Full QBM complete on 9 general-funnel finalists (14 Aug) plus 11 small/mid-cap Health Care finalists (19 Aug); 10 Add decisions stand, of which **6 hold a slot in the concentrated target portfolio** (MA, LLY, VCYT, PTCT, XNCR, ACAD) and **4 sit on the bench** (NEO, HRMY, KRYS, HQY); no capital deployed yet |
...
- Methodology version: 1.6
- Investment Charter version: 1.2
...
- Operating-system release: QBM-US Version 1.9
